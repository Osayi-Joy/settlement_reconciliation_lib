 ...
 *     <p>Configures beans required for JWT signing and validation
 */
@Slf4j
@Configuration
...
public class JwtConfiguration {
  ...
  public KeyStore keyStore() {
    try {
      ...
      keyStore.load(
          resourceAsStream, propertyConfig.getSecurityJWTKeyStorePassword().toCharArray());
      return keyStore;
  ...
  public RSAPrivateKey jwtSigningKey(KeyStore keyStore) {
    try {
      Key key =
          keyStore.getKey(
              propertyConfig.getSecurityJWTKeyAlias(),
              propertyConfig.getSecurityJWTPrivateKeyPassphrase().toCharArray());
      ...
      log.error(
          ...
          propertyConfig.getSecurityJWTKeyStorePath(),
          e);
    }
